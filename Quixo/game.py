...
class Game(object):
    ...
    def to_state(self):
        ...
        return GameState(self._board, self.current_player_idx, winner)
    ...
    def play(self, player1: Player, player2: Player) -> int:
        ...
        winner = -1
        turn_count = 0
        while winner < 0 and turn_count < MAX_GAME_STEPS:
            self.current_player_idx += 1
            ...
            if self.current_player_idx == len(players) - 1:
                turn_count+=1
                self.moves_count += 1   
            
            winner = self.check_winner()
        
        sequence = self.get_winning_sequence()  
        draw = (winner<0) and (turn_count>=MAX_GAME_STEPS)

        
        return winner, sequence, draw
    ...
    def get_winning_sequence(self) -> dict:
        ...
        if self._board[0, 0] != -1 and all(self._board[x, x] == self._board[0, 0] for x in range(self._board.shape[0])):
            return {"type": "diag"}

        # Controllo per la diagonale secondaria
        if self._board[0, -1] != -1 and all(self._board[x, -(x + 1)] == self._board[0, -1] for x in range(self._board.shape[0])):
