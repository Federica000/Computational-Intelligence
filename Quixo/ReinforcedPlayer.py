...
class ReinforcedPlayer(Player):
    ...
    def save(self):
        np.save(f"./Computational_Intelligence/Quixo/reinforced_player_{self.GAME_NUMBER}.npy", self)
    
    def load(self):
        saved_data = np.load(f"./Computational_Intelligence/Quixo/reinforced_player_{self.GAME_NUMBER}.npy", allow_pickle=True).item()
        self.Q = saved_data.Q
